...
for n in range(numpairs):
    ...
    # Also, randomize the order to eliminate any exposure time trend.
    if n % 2 == 0:
        if rand() > 0.5:
            ccd1=MaskedCCD(patterns[2 * n])
            ccd2=MaskedCCD(patterns[2 * n + 2])
        else:
            ccd2=MaskedCCD(patterns[2 * n])
            ccd1=MaskedCCD(patterns[2 * n + 2])
    else:
        if rand() > 0.5:
            ccd1=MaskedCCD(patterns[2 * n - 1])
            ccd2=MaskedCCD(patterns[2 * n + 1])
        else:
            ccd2=MaskedCCD(patterns[2 * n - 1])
            ccd1=MaskedCCD(patterns[2 * n + 1])
    ...
    for segment in range(numsegments):
        extname=pf.getheader(patterns[n],segment+1)['EXTNAME'] 
        if extname in []: # Skip bad segments
            continue
        amp=segment + 1
        image1 = ccd1.unbiased_and_trimmed_image(amp)
        image2 = ccd2.unbiased_and_trimmed_image(amp)
        med1 = np.median(image1.getArrays()[0])
        med2 = np.median(image2.getArrays()[0])
        ...
        mask1 = np.ma.masked_where(abs(image1.getArrays()[0] - med1) > thresh1,image1.getArrays()[0],copy=True)
        print "Mask1 masking out %d pixels"%mask1.mask.sum()
        var2 = med2
        thresh2 = Nsigma*sqrt(var2)
        mask2 = np.ma.masked_where(abs(image2.getArrays()[0] - med2) > thresh2,image2.getArrays()[0],copy=True)
        ...
        # Add the edge mask
        fdiff.mask=np.ma.mask_or(expanded_mask,edges)
        (nrows,ncols)=fdiff.shape
        print "Diff Masking out %d pixels, including edges"%fdiff.mask.sum()
        print "For file %d, segment = %s,image1 median = %.2f, image2 median = %.2f"%(n,extname, np.median(image1.getArrays()[0]), np.median(image2.getArrays()[0]))
        print "Diff min = %f, max = %f, mean = %f, median = %f"%(fdiff.min(), fdiff.max(), fdiff.mean(), median(fdiff))
